...
-- (9) 회원탈퇴처리
COMMIT;
UPDATE MEMBER SET MWITHDRAWAL=0 WHERE MID='bbb';
ROLLBACK;
----------------------------------------------------------------------
-----------------------------  ADMIN_DAO  ----------------------------
----------------------------------------------------------------------
...
-- (2) 1번글에 달린 댓글 모두 출력
SELECT * FROM GCOMMENT WHERE GNUM=1 ORDER BY GCDATE;
-- (3) 댓글 수정
UPDATE GCOMMENT SET GCCONTENT = '수정했는데도 좋네요' WHERE GCNUM=1;
-- (4) 댓글 삭제
COMMIT;
...
SELECT V.*, MNICKNAME FROM VBOARD V, MEMBER M  WHERE V.MID=M.MID AND M.MID='aaa';
-- (6) 글 수정하기
UPDATE VBOARD 
    SET CNAME = '가렌',
        VTITLE = '가붕이돈다',
...
-- (2) 1번글에 달린 댓글 모두 출력
SELECT * FROM VCOMMENT WHERE VNUM=1 ORDER BY VCDATE;
-- (3) 댓글 수정
...
SELECT F.*, MNAME FROM FBOARD F, MEMBER M  WHERE F.MID=M.MID AND FNUM=1;
...
UPDATE FBOARD SET FSTEP = FSTEP + 1 WHERE FGROUP=2 AND FSTEP>0;
-- (9) 답변글 쓰기
INSERT INTO FBOARD (FNUM, FTITLE, FCONTENT, FIMAGE, FGROUP, FSTEP, FINDENT, MID, FIP)
    VALUES (FBOARD_FNUM_SEQ.NEXTVAL, '답변이 가능한', '더 자유로운 자유게시판', NULL, 2, 1, 1, 'aaa', '111.1.1.1');
COMMIT;
----------------------------------------------------------------------
...
-- (2) 1번글에 달린 댓글 모두 출력
SELECT * FROM FCOMMENT WHERE FNUM=1 ORDER BY FCDATE;
-- (3) 댓글 수정
