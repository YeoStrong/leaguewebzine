...
public class MemberDao {
	...
	public int joinMember(MemberDto member) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "INSERT INTO MEMBER (MID, MNICKNAME, MPW, MNAME, MTEL, MBIRTH, MEMAIL, MGENDER, MPHOTO, MADDRESS)" + 
				"    VALUES (?, ?, ?, ?, ?, ?, ? , ?, ?, ?)";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			...
			pstmt.setString(9, member.getMphoto());
			pstmt.setString(10, member.getMaddress());
	...
	public MemberDto getMember(String mid) {
		...
		PreparedStatement pstmt = null;
		ResultSet         rs    = null;
		String sql = "SELECT M.*, LNAME FROM MLEVEL L, MEMBER M" + 
				"    WHERE M.LLEVELNUM=L.LLEVELNUM AND MID=?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mid);
			...
			if(rs.next()) {
				...
				int llevelnum = rs.getInt("llevelnum");
				int mwithdrawal = rs.getInt("mwithdrawal");
				member = new MemberDto(mid, mnickname, mpw, mname, mtel, mbirth, memail, mgender, mphoto, maddress, mrdate, llevelnum, mwithdrawal);
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}finally {
	...
	public int modifyMember(MemberDto member) {
		...
		try {
			...
			pstmt.setString(8, member.getMphoto());
	...
	public ArrayList<MemberDto> getMemberlist(int startRow, int endRow) {
		...
		try {
			...
			while(rs.next()) {
				...
				int llevelnum = rs.getInt("llevelnum");
				int mwithdrawal = rs.getInt("mwithdrawal");
				members.add(new MemberDto(mid, mnickname, mpw, mname, mtel, mbirth, memail, mgender, mphoto, maddress, mrdate, llevelnum, mwithdrawal));
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}finally {
	...
	public int withdrawalMember(String mid) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE MEMBER SET MWITHDRAWAL=0 WHERE MID=?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mid);
