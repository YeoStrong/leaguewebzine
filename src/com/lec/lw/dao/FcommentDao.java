...
public class FcommentDao {
	...
	public ArrayList<FcommentDto> listFcomment(int fnum){
		...
		PreparedStatement pstmt = null;
		ResultSet         rs    = null;
		String sql = "SELECT F.*, MNICKNAME FROM FCOMMENT F, MEMBER M WHERE M.MID=F.MID AND FNUM=? ORDER BY FCDATE";
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, fnum);
	...
	// (3) 댓글 수정 view
	public FcommentDto fcommentModifyView(int fcnum) {
		FcommentDto dto = null;
		Connection      conn  = null;
		PreparedStatement pstmt = null;
		ResultSet         rs    = null;
		String sql = "SELECT F.*, MNICKNAME FROM FCOMMENT F, MEMBER M WHERE M.MID=F.MID AND FCNUM=?";
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			...
			if(rs.next()) {
				...
				String mnickname = rs.getString("mnickname");
				Timestamp fcdate = rs.getTimestamp("fcdate");
				dto = new FcommentDto(fcnum, mid, mnickname, fnum, fccontent, fcdate);
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		}finally {
